...
class Dataloader:
    def __init__(self, train_pos, train_neg, kg_lines, train_batch_size: int = 128, neg_rate: float = 2):
        ...
        self.rel_num = len(self.rel_dict)  + 1
...
class TransE(torch.nn.Module):
    def __init__(self, ent_num: int, rel_num: int, dataloader: Dataloader, dim: int = 128, l1: bool = True,
                 margin: float = 15, learning_rate: float = 1e-3, weight_decay: float = 1e-4, device_index: int = 0):
        super().__init__()
...
class KGRS:
    def __init__(self, train_pos: np.array, train_neg: np.array, kg_lines: List[str]):
        ...
        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 0.3, "emb_dim": 128, "l1": True, "margin": 32,
                  "learning_rate": 8e-4, "weight_decay": 8e-3, "epoch_num": 30}
        print(config)
