...
def thread_val(iem,sol:tuple[set,set], n):
    tmp=[]
    for _ in range (n):
        r1,ac1 = iem.prop(iem.g,'c1', sol)
        r2,ac2 = iem.prop(iem.g,'c2', sol)
        delta = r1.difference(r2).union(r2.difference(r1)) 
        tmp.append(len(iem.og.nodes) - len(delta))
...
class IEM:
    ...
    def prop(self,g:DiGraph,si:set,ci, exposed:set, activated:set) -> tuple[set,set]:
        queue = Queue()
    ...
    def getOrigin(self, s1:set,s2:set) -> list[tuple[set,set,set,set,int]]:
        for i in range(self.N):
            ac = set()
            ex = set()
            ex1,ac1 = self.prop(self.g,s1,'c1',ac,ex)
            ac = set()
            ex = set()
            ex2,ac2 = self.prop(self.g,s2,'c2',ac,ex)
            delta = ex1.difference(ex2).union(ex2.difference(ex1)) 
            score = len(self.og.nodes) - (len(delta))
            self.preset.append((ex1,ac1,ex2,ac2,score))
    ...
    def __init__(self, g: DiGraph, og: DiGraph, i1:set, i2:set,neib:dict, sample_N, K:int) -> None:
        ...
        self.K = K
        self.preset = []
        self.getOrigin(self.i1, self.i2)
    ...
    def sample(self, sol, preset):
        ex1,ac1,ex2,ac2,score = preset
        s1,s2 = sol
        r1,ac1 = self.prop(self.g,s1,'c1',ex1,ac1)
        r2,ac2 = self.prop(self.g,s2,'c2',ex2,ac2)
        delta = r1.difference(r2).union(r2.difference(r1)) 
        return len(self.og.nodes) - (len(delta))
    
    def value(self, sol:tuple[set,set] ):
        tmp=[]
        for preset in self.preset:
            tmp.append(self.sample(sol,preset))
        return np.average(tmp)

    # def value_mp(self, sol:tuple[set,set]) -> int:
    #     tmp = []
    #     pool = mp.Pool(processes=THREADS)
    #     cnt = self.N//THREADS
    #     for _ in range(THREADS):
    #         tmp.append(pool.apply_async(thread_val, args=(self,sol,cnt)))
    #     pool.close()
    #     pool.join()
    #     score = []
    #     for i in tmp:
...
class EA:
    ...
    def select_parents(self, population):
        ...
        min_fit = min(fit)
        fit = [(i - min_fit)**3 + 1 for i in fit]
        # Create roulette wheel.
        sum_fit = sum(fit)
        wheel = list(accumulate([i/sum_fit for i in fit]))

        father_index, mother_index = 0,0
        father_index = bisect_right(wheel, rand.random()) % len(info)
        father = info[father_index]

        mother_index = bisect_right(wheel, rand.random()) % len(info)
        mother = info[mother_index]
    ...
    def crossover(self, parent1, parent2):
        ...
        for i in range(int(r * self.crossover_rate)):
            if prob(0.5):
    ...
    def mutate(self, individual):
        ...
        for i in range(int(self.problem_instance.K * self.mutation_rate )):
            if len(s1_) > 0:
                nodeA = rand.choice(list(s1_))
            else:
                nodeA = None
            nodeB = rand.choice(list(pool_1))
            if prob(self.special_rate) or not nodeA:
                if prob(0.5):
    ...
    def evolve(self):
        ...
        for generation in range(self.generations):
            ...
            ed = time.time()
            if (ed-start >= TLE): break
            print("{}    score {}   solution:{}, time cost {}s".format(generation, best_fitness.__round__(2), best_solution[0], (ed-st).__round__(2)))
        end = time.time()
...
if __name__=='__main__':
    iem = IEM.create(NET_WORK, SEED, cut_off=0.05, sample_N=18, K=5)
